...
function normalize(value:Partial<V2Save>|undefined,slot:number):V2Save{
  ...
  delete (settings as unknown as Record<string,unknown>).skipBossStage;delete (settings as unknown as Record<string,unknown>).bonusRewind;delete (settings as unknown as Record<string,unknown>).extraDash;delete (settings as unknown as Record<string,unknown>).heatHud;delete (settings.bindings as Record<string,string>).rewind;delete (settings.bindings as Record<string,string>).anchor;delete (settings.bindings as Record<string,string>).dash;
...
function renderStory(room:typeof rooms[number]):void{
  const story=room.story,box=$('room-story');
  box.classList.toggle('hidden',!story);
  if(!story)return;
  setText('room-story-speaker',story.speaker);
  setText('room-story-text',story.text);
}
...
const game=new IWannaGame($('game-root'),{
  onHud:data=>{
    ...
    setText('beat-stamps',data.segments.total?`${data.segments.current} / ${data.segments.total}`:'—');const segmentTitle=data.segments.total?`已完成 ${data.segments.completed} 段，共 ${data.segments.total} 段`:'当前房间不分段';if($('beat-stamps').title!==segmentTitle)$('beat-stamps').title=segmentTitle;setText('segment-goal',`${segmentRoleLabel(data.segments.role)} · ${data.segments.label}`);
    setText('jumps','●'.repeat(data.jumps)+'○'.repeat(2-data.jumps));setText('echoes',`${data.echoes} / ${data.director.echoLimit}`);setText('shards',`${data.shards} / 12`);setText('deaths',String(data.deaths));setText('timer',formatTime(data.elapsed));
    ...
    setText('director-command',data.boss.active?`Boss ${data.boss.phase}/${data.boss.max} · 攻势 ${Math.min(data.boss.waves,data.boss.required)}/${data.boss.required}`:data.lock.remaining?`下一机关：${data.lock.next}锁`:`规则：${data.director.label}`);if(hud.dataset.mode!==data.mode)hud.dataset.mode=data.mode;hud.classList.toggle('hidden',current.settings.hudMode==='hidden');
  },
  onRoom:room=>{const card=$('room-card');window.clearTimeout(roomCardTimer);$('chapter-label').textContent=room.kind==='boss'?'章节终局 · 完整阶段战':room.kind==='prologue'?'序章 · 基本操作':room.kind==='epilogue'?'终章':room.lesson?`第 ${room.chapter} 章 · 第 ${room.lesson.step}/${room.lesson.total} 关 · ${room.lesson.tier}`:`第 ${room.chapter} 章`;$('room-title').textContent=room.name;renderStory(room);const bill=room.remixKind?stageBill(room.remixKind):'';const storyGoal=room.story?.objective??'';$('room-hint').textContent=room.kind==='boss'?`${room.hint}｜每阶段先躲过规定攻击波，机关才会解锁。`:room.lesson?`本关课题：${room.lesson.focus}。${room.attackTheme?`远程威胁：${room.attackTheme}。`:''}`:room.hint;$('pause-objective').textContent=storyGoal|| (room.kind==='boss'?'Boss 阶段不能跳过：观察顶部攻势计数，躲满两至三波攻击，再按顺序触发当前阶段机关。':room.lesson?`第 ${room.lesson.step}/${room.lesson.total} 关（${room.lesson.tier}）。依次完成学习、练习、考验、终点四段；本关只组合：${bill||room.lesson.focus}。`:'目标：沿清晰路线抵达出口。');card.classList.remove('hidden');card.style.animation='none';void card.offsetHeight;card.style.animation='';roomCardTimer=window.setTimeout(()=>card.classList.add('hidden'),4500);},
  ...
  onRoomResult:r=>toast(`${r.rank} 评级 · ${formatTime(r.time)} · ${r.deaths} 死亡`),
  onContract:result=>toast(result.success?`悬赏完成：${result.label} · 封印 ${result.seals}/24`:`悬赏未完成：${result.label}`),
  onBeat:result=>{const flash=$('beat-flash');window.clearTimeout(beatFlashTimer);$('beat-flash-label').textContent=`第 ${result.index} 段完成 · ${result.label}`;$('beat-flash-rank').textContent=`${segmentRoleLabel(result.role)}完成 · 继续前进`;flash.classList.remove('gold');flash.classList.remove('hidden');flash.style.animation='none';void flash.offsetHeight;flash.style.animation='';beatFlashTimer=window.setTimeout(()=>flash.classList.add('hidden'),1100);},
...
function showEnding(kind:EndingId,save:V2Save):void{releaseAllTouchActions();hud.classList.add('hidden');show(ending);const copy={escape:['逃离结局','你只带走了少量记忆，先离开了乐园。园长的广播仍在循环，因为你还不知道自己曾经启动过它。'],takeover:['接管结局','你看懂了实验，却选择戴上园长的胸牌。你控制了循环，却也继承了它。'],destroy:['真结局','十二段记忆拼回了完整真相：你是设计师，园长是执行人格。你关闭总电闸，停止实验，并承认机器造成的伤害。'],accept:['隐藏结局 · 带着记忆生活','你没有删除痛苦，也没有继续扮演园长。你带着完整记录走向现实，准备为过去负责。']}[kind];$('ending-title').textContent=copy[0];$('ending-text').textContent=copy[1];$('ending-stats').innerHTML=`<div><b>${save.shards.length}/12</b>核心记忆</div><div><b>${save.notes.length}</b>可选档案</div><div><b>${formatTime(save.elapsed)}</b>总时间</div>`;updateTouchControls();}
...
function renderArchive():void{const notes=rooms.flatMap(r=>r.optional??[]),records=Object.entries(current.bestRooms).sort((a,b)=>a[0].localeCompare(b[0]));$('archive-list').innerHTML=[
  ...
  '<h3 class="archive-heading">房间成绩与最佳残影</h3>',...(records.length?records.map(([id,r])=>`<div class="archive-entry"><b>${rooms.find(x=>x.id===id)?.name??id} · ${r.rank}</b>${formatTime(r.time)} · 最少 ${r.deaths} 死亡${r.contract?' · ✓ 额外挑战':''}${current.ghostRooms[id]?.length?' · 已记录残影':''}${r.assisted?' · 辅助':''}</div>`):['<div class="archive-entry locked"><b>暂无成绩</b>通过房间后会记录评级与最佳路线残影。</div>'])
].join('');}
