...
const slotsScreen=$('slots-screen'),chaptersScreen=$('chapters-screen'),archiveScreen=$('archive-screen'),memoryFlash=$('memory-flash');
...
if(new URLSearchParams(location.search).get('debug')==='1')Object.assign(window,{__lostMemoryParkDebug:{snapshot:()=>game.debugSnapshot(),startRoom:(index:number)=>game.startAtRoom(index),teleport:(x:number,y:number)=>game.debugTeleport(x,y),bossPhase:(phase:number)=>game.debugForceBossPhase(phase),bossWaves:(waves:number)=>game.debugSetBossWaves(waves),win:()=>game.debugWinCurrentRoom(),setCollections:(shards:number,notes:number)=>game.debugSetCollections(shards,notes),choose:(side:'left'|'right')=>game.debugChooseEnding(side),overlay:(visible:boolean)=>game.debugSetOverlay(visible),down:(action:'left'|'right'|'jump'|'restart')=>game.actionDown(action),up:(action:'left'|'right'|'jump')=>game.actionUp(action),save:()=>game.getSave()}});

type TouchAction='left'|'right'|'jump';
...
async function enter(kind:'new'|'continue',room?:number):Promise<void>{await game.audio.unlock();inTitle=false;hide(title);hide(ending);hud.classList.remove('hidden');if(kind==='new'){const fresh=normalize(undefined,currentSlot);fresh.started=true;fresh.settings={...current.settings,bindings:{...current.settings.bindings}};saves[currentSlot]=fresh;current=fresh;game.setSave(current);game.startNew();}else if(room!==undefined)game.startAtRoom(room);else game.continueGame();updateTouchControls();}
async function enterMode(mode:V2Save['mode']):Promise<void>{await game.audio.unlock();inTitle=false;hide(title);hide(ending);hide($('modes-screen'));hud.classList.remove('hidden');game.startMode(mode);updateTouchControls();}
function backTitle():void{releaseAllTouchActions();game.returnToTitle();inTitle=true;hide(pause);hide(ending);hud.classList.add('hidden');show(title);refreshContinue();updateTouchControls();}
...
const bindingLabels:Record<string,string>={left:'向左',right:'向右',jump:'跳跃',restart:'重开',pause:'暂停'};
...
let gpState={left:false,right:false,jump:false,pause:false};function pollGamepad(){if(current.settings.gamepad&&!inTitle){const pad=navigator.getGamepads?.()[0];if(pad){const next={left:pad.axes[0]<-.25||pad.buttons[14]?.pressed,right:pad.axes[0]>.25||pad.buttons[15]?.pressed,jump:pad.buttons[0]?.pressed,pause:pad.buttons[9]?.pressed??false};for(const [key,action] of [['left','left'],['right','right'],['jump','jump']] as const){if(next[key]&&!gpState[key])game.actionDown(action);if(!next[key]&&gpState[key])game.actionUp(action);}if(next.pause&&!gpState.pause)togglePause();gpState=next;}}requestAnimationFrame(pollGamepad)}pollGamepad();
