...
![Version](https://img.shields.io/badge/version-12.4.0-d5a546)
![TypeScript](https://img.shields.io/badge/TypeScript-strict-3178c6)
![Tests](https://img.shields.io/badge/tests-90%20passed-3f8378)

## 立即游玩

**永久公开地址：** <https://liuzhanrui30-boop.github.io/lost-memory-park/>

![《失忆乐园》v12.3 高清实机画面](docs/images/gameplay-v12.3.png)

![《失忆乐园》v12.4 手机横屏触控界面](docs/images/mobile-v12.4.png)

根目录的 [`index.html`](index.html) 是已经内嵌 CSS、JavaScript 与全部游戏内容的离线单文件版本。下载后双击即可运行，不需要服务器、账号或联网资源。

## 一句话介绍

玩家控制失去颜色与记忆的白色纸偶，在由“笑脸园长”管理的失控游乐园中，只依靠左右移动、双跳与快速重开，通过会欺骗玩家的舞台机关，找回 12 段核心记忆并决定乐园的结局。

## 玩家承诺

- **手感明确：** 操作只有移动、双跳、重开，但每个动作都可精确控制。
...
| 快速重开 | `R` | 回到最近检查点 |
| 清除残影 | `Backspace` | 清空死亡残影并重置本房间机关 |
| 暂停 | `Esc` | 暂停模拟与音频 |

手机/平板横屏进入游戏后会自动显示左右移动、跳跃和暂停按钮。方向区支持按住滑动换向，移动与跳跃可多点同时按下；设置中可以调整自动显示/始终显示/关闭、方向键大小、跳跃键大小、透明度、底部距离与左右手布局。

> 当前正式规则中没有冲刺、攻击、记忆锚点或回溯技能。关卡必须只靠移动和双跳完成。
...
| 自动测试 | 24 个测试文件，90 项测试 |

## 双端画质与性能

v12.4 按设备使用两档明确的内部画布，不让浏览器盲目按 Retina 倍率无限增加填充量：
...
7. [测试、调试与性能](docs/07-测试调试与性能.md)：90 项测试、Debug API、双端性能预算和回归流程。
...
```mermaid
flowchart LR
    Input[键盘/手柄/触屏] --> Main[src/main.ts UI协调层]
...
v12.4 是可在电脑与手机横屏完整游玩、可离线运行、可公开分享的版本。未来方向记录在 GDD 的“开放问题与后续范围”中；未写入当前范围的多人、服务器排行榜、云存档和商业付费系统不属于当前实现。
