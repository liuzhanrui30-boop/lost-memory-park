...
> 文档状态：对应 v12.9.2 实际实现。本文是设计、程序、关卡、叙事和 QA 的共同事实来源；与旧设想冲突时，以当前代码和本文为准。
...
剧情不再依赖玩家自行拼接零散档案。每次进入一个房间，房间卡按“说话者 → 线索 → 当前目标”显示短故事节点；暂停页保留当前目标。四章的因果顺序固定为：

1. **身份：** 白偶其实是乐园设计师抹掉记忆后的玩家版本。
2. **实验：** 乐园把痛苦加工成关卡，用死亡和重来测试服从。
3. **园长：** 笑脸园长是设计师留下的执行人格，因此他既是对手，也是主角过去的决定。
4. **选择：** 记忆不是能力升级，而是让主角决定逃离、接管、关闭实验，或带着责任回到现实的证据。
